...
class Strawberry():
    ...
    def random_pos(self, snake):
        ...
        self.position[0] = random.randint(27, 57)  # position the food appear
        self.position[1] = random.randint(27, 57)
...
class Game:
    ...
    def do_move(self, move):
        ...
        self.snake.update()
        pygame.mixer.init()
        score_sound = pygame.mixer.Sound('./sound/laugh.wav')  # add score sound effect
        if self.snake.position == self.strawberry.position:
            pygame.mixer.Sound.play(score_sound)
            self.strawberry.random_pos(self.snake)
            reward = 1
            self.snake.score += 1
            self.x = self.snake.score
            if self.snake.score >= 5:
    ...
    def game_end(self):
        ...
        if self.snake.score >= 5:
            if self.snake.segments[0] in self.snake.segments[1:]:  # give more chance if the snake hit himself while the score >= 6
                self.snake.score -= 5
                self.chance -= 1
                self.snake.segments.pop(-1)
                self.snake.segments.pop(-1)
                self.snake.segments.pop(-1)
                self.snake.segments.pop(-1)
                self.snake.segments.pop(-1)
    ...
    def blit_extra_chance(self, color, screen):
        font = pygame.font.SysFont('mvboli', 60)  # font type and size
        text = font.render('Extra Chance: ' + str(self.chance), True, color)
